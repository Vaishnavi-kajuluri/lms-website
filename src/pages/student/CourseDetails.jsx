  ...
  return courseData ? (
    <>
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
        ...
        {/* Background gradient */}
        <div className='absolute top-0 left-0 w-full h-[500px] z-0 bg-gradient-to-b from-cyan-100/70' />

        {/* Foreground content */}
        <div className='relative z-10 max-w-xl text-gray-500 -mt-10'>
            ...
            {/* ratings &reviews*/}
             <div className='flex items-center space-x-2'>
                      ...
                      <div className='flex'>
                        ...
                        (calculaterating(courseData)) ? assets.star:assets.star_blank}
                         alt='' className='w-3.5 h-3.5'/> 
                      ))}
                      </div>
                      <p className='text-blue-500'>({courseData.courseRatings.length} {courseData.
           ...
           <div className='pt-5'>
             {courseData.courseContent.map((chapter,index)=>(
                ...
                rounded'>
                  ...
                  px-4 py-3 cursor-pointer select-none' onClick={()=>toggleSection(index)}>
        ...
        <div className='py-20 text-sm md:text-default'>
          <h3 className='text-xlfont-semibold text-gray-800'>Course Description</h3>
          <p className='pt-3 rich-text'
              dangerouslySetInnerHTML={{__html:courseData.courseDescription}}></p>
        </div>
        </div>

        {/* Right column (reserved for image, etc.) */}
        <div className='max-w-[424px] z-10 rounded-t md:rounded-none
         overflow-hidden bg-white 
        min-w-[300px] sm:min-w-[420px]'  style={{ boxShadow: '0 4px 15px 2px  rgba(0, 0, 0, 0.8)' }}>
          {
             playerData ? 
          ...
          <div className='p-5'>
            ...
            <div className='flex gap-3 items-center pt-2'>
              <p className='text-gray-800 md:text-4xl text-2xl font-semibold'>{currency}{(courseData.coursePrice-courseData.discount*courseData
                .coursePrice/100).toFixed(2)}</p>
              <p className='md:text-lg text-gray-500 line-through'>{currency}{courseData.coursePrice}</p>
              <p className='md:text-lg text-gray-500'>{courseData.discount}% off</p>
            </div>
            <div className='flex items-center text-sm md:text-default gap-4 
            pt-2 md:pt-4 text-gray-500'>
            ...
            <button className='md:mt-6 mt-4 w-full py-3 rounded
            bg-blue-600 text-white font-medium'>{isAlreadyEnrolled ? 'Already Enrolled' :'Enroll Now'}</button>
           <div>
            <p>What's in the course?</p>
            <ul className='ml-4 pt-2 text-sm md:text-default 
            list-disc text-gray-500'>
