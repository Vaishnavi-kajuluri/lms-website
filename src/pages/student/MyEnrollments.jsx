        ...
        <tbody className='text-gray-800'>
          {enrolledCourses.map((course,index)=>(
            <tr key={index} className=''>
              <td className='md:px-4 pl-2
              md:pl-4 py-3 flex items-center space-x-3'>
              ...
              <td className='px-4 py-3 max-sm:hidden'>
               ...
               lectureCompleted} /${progressArray[index].totalLectures}`} <span>Lectures</span>          
              </td>
              <td className='px-4 py-3 max-sm:text-right'>
                <button className='px-4 sm:px-5 py-1.5 sm:py-2 bg-blue-600
                max-sm:text-xs cursor-pointer rounded text-white' onClick={()=>navigate('/player/'+course._id)}>
